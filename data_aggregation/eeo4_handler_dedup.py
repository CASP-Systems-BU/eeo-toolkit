...
Note: Table C is considered as the subset of Table A (new hires are a subset of
full-time staff), and thus we ignore data in Table C in this script to avoid
double-counting.
"""
...
df.to_csv(os.path.join(output_dir, "eeo4_dedup.csv"), index=False)

# === Reload and print county-level summary ===
df = pd.read_csv(os.path.join(output_dir, "eeo4_dedup.csv"))
