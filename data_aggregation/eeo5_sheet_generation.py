"""
This script post-processes differentially private EEO-5 contingency tables.
It expects the outputs of eeo5_dp.py (WCRG.csv, JRG.csv, TWR.csv, TWG.csv,
TJR.csv, TJG.csv) and produces:
1. Raw and zeroized Excel workbooks for initial review.
2. Adjusted 3-way side tables with non-negative counts (cvxpy MIP).
3. An adjusted 4-way WCRG table (Work_Type × Job_Category × Race × Gender).
...
# Directory containing WCRG.csv, JRG.csv, TWR.csv, TWG.csv, TJR.csv, TJG.csv
...
def create_about_sheet_eeo5(filename):
    """Prepend an About sheet with a table of contents to an existing EEO-5 workbook."""
    sheet_descriptions = {
        'Category-Race-Sex':        'Job Category × Race/Ethnicity × Gender',
        'Work-Category-Race-Sex':   'Work Type × Job Category × Race/Ethnicity × Gender',
        'Agent-WorkType-Race':      'Agent Type × Work Type × Race/Ethnicity',
        'Agent-WorkType-Sex':       'Agent Type × Work Type × Gender',
        'Agent-Category-Race':      'Agent Type × Job Category × Race/Ethnicity',
        'Agent-Category-Sex':       'Agent Type × Job Category × Gender',
    }
    ...
    ws['A2'] = (
        'This data has been collected from EEO-5 reports submitted by Massachusetts public '
        'school districts under the Frances Perkins Workplace Equity Act of 2024. Several '
        'steps were taken to protect data and identity of respondents. These include physical '
        'and logical access controls, cryptographically secure multi-party computation to '
        'provide end-to-end data confidentiality, and differential privacy techniques. In '
        'particular, all counts shown in these tables are approximate. More information about '
        'data security can be found in the methodology section of the full 2026 Massachusetts '
        'Workforce Data Report. A table of contents is below.'
    )
...
wcrg = pd.read_csv(os.path.join(input_dir, "WCRG.csv"))
raw_tables = [pd.read_csv(os.path.join(input_dir, f))
              for f in ["JRG.csv", "TWR.csv", "TWG.csv", "TJR.csv", "TJG.csv"]]
raw_tables.append(wcrg)
for i in range(len(raw_tables)):
    raw_tables[i].rename(columns=RENAME_DICT, inplace=True)

dataframes_raw = {
    'Category-Race-Sex':      raw_tables[0],
    'Agent-WorkType-Race':    raw_tables[1],
    'Agent-WorkType-Sex':     raw_tables[2],
    'Agent-Category-Race':    raw_tables[3],
    'Agent-Category-Sex':     raw_tables[4],
    'Work-Category-Race-Sex': raw_tables[5],
}

out_raw = os.path.join(output_dir, "MA_Workforce_2026_EEO5_raw.xlsx")
write_tables_with_titles(out_raw, dataframes_raw)
create_about_sheet_eeo5(out_raw)
...
out_zeroized = os.path.join(output_dir, "MA_Workforce_2026_EEO5_remove_all_zeroes.xlsx")
write_tables_with_titles(out_zeroized, dataframes_zeroized)
create_about_sheet_eeo5(out_zeroized)
...
# Reads raw CSVs from disk (original column names from eeo5_dp.py).
...
THREE_WAY_CONFIGS = [
    ...
    {"file": "TWR.csv", "max_cell": 1, "max_1way": 11, "max_2way": 13},
    {"file": "TWG.csv", "max_cell": 1, "max_1way": 11, "max_2way": 13},
    {"file": "TJR.csv", "max_cell": 2, "max_1way": 10, "max_2way": 20},
    {"file": "TJG.csv", "max_cell": 2, "max_1way": 10, "max_2way": 20},
]
...
# =============================================================================
# Step 3: Adjust the 4-way WCRG table
# =============================================================================

adjust_4way_table(
    csv_path=os.path.join(input_dir, "WCRG.csv"),
    count_col='Count',
    use_l1=True,
    max_cell_change=3,
    ...
    output_csv=os.path.join(output_dir, "WCRG_adj.csv"),
)
print("→ saved WCRG_adj.csv")
...
dfs_adj = []
for filename in ["JRG_adj.csv", "TWR_adj.csv", "TWG_adj.csv", "TJR_adj.csv", "TJG_adj.csv",
                 "WCRG_adj.csv"]:
...
dataframes_adj = {
    ...
    'Agent-WorkType-Race':    dfs_adj[1],
    'Agent-WorkType-Sex':     dfs_adj[2],
    'Agent-Category-Race':    dfs_adj[3],
    'Agent-Category-Sex':     dfs_adj[4],
    'Work-Category-Race-Sex': dfs_adj[5],
}

out_adj = os.path.join(output_dir, "MA_Workforce_2026_EEO5_adjusted.xlsx")
write_tables_with_titles(out_adj, dataframes_adj)
create_about_sheet_eeo5(out_adj)
