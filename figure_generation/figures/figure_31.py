...
from eeo5_figure_config import BAR_EDGE_COLOR, PIE_ORDER, RACE_COLORS


def figure_31(jrg_df, output_dir, table_dir):
    EXPLODE = 0.06
    pie_df  = jrg_df.groupby('Race')['Count'].sum().reindex(PIE_ORDER).fillna(0).clip(lower=0)
    total   = pie_df.sum()
    ...
    fig, ax = plt.subplots()
    patches, _ = ax.pie(
        pie_df, labels=pie_labels, textprops={'fontsize': 12},
    ...
    # Export Figure
    fig.savefig(f'{output_dir}/figure_31.png', dpi=300, bbox_inches='tight')
    plt.close(fig)

    # Create Table
    fig31_df = pd.DataFrame({
        'Race':       PIE_ORDER,
        'Percentage': (pie_df.values / total * 100).round(0).astype(int).astype(str) + '%',
    })
    # Export Table
    fig31_df.to_csv(f'{table_dir}/figure_31_table.csv', index=False)
