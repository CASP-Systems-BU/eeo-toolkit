...
from eeo1_figure_config import FEMALE_COLOR, MALE_COLOR, BAR_EDGE_COLOR

def figure_1(df, output_dir, table_dir):
    ...
    # Export Figure
    fig.savefig(f'{output_dir}/figure_1.png', dpi=300, bbox_inches='tight')
    plt.close(fig)

    # Create Table
    fig1_df = pd.DataFrame({
        'Sex': ['Female', 'Male'],
        'Percentage': [gender_totals['Female']/(gender_totals['Female']+gender_totals['Male']), gender_totals['Male']/(gender_totals['Female']+gender_totals['Male'])]
    })
    fig1_df['Percentage'] = (fig1_df['Percentage'] * 100).round(0).astype(int).astype(str) + '%'

    # Export Table
    fig1_df.to_csv(f'{table_dir}/figure_1_table.csv', index=False)