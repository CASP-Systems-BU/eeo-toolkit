...
This script automates the process of re-rendering PDF files using Firefox in headless mode.
It is used to fix visual misplacements in form elements (e.g., checkboxes/text not aligned) by
reprinting PDFs through the browser's rendering engine.

Steps:
1. Load all PDF files in a given input folder.
2. Open each PDF in Firefox and trigger the "Print to PDF" function.
3. Save the newly rendered PDF to an output folder.
4. Skips already-processed files, logs all activities, and periodically restarts Firefox to reduce crashes.
"""
...
def wait_for_file(file_path, timeout=1000):
    """
    Wait until the file is created and has non-trivial size.
    ...
    Returns:
        bool: True if file is found and large enough, else False.
    """
