...
def update_diff_cnt(diff_cnt, json_data, table_name, file):
    ...
    if col_sum != whole_total:
        diff = max(diff, abs(col_sum - whole_total))
    if diff != 0:
        diff_cnt[table_name] = diff_cnt.get(table_name, {})
        diff_cnt[table_name][diff] = diff_cnt.get(table_name, {}).get(diff, 0) + 1
    if diff > 10:
        print(file, diff)
...
if __name__ == "__main__":
    ...
    # modify output_dir if needed. By default, it would be input_dir
